...
const structuredLogColumns = computed(() => [
  ...
  },
  {
    ...
    customRender: ({ record }: { record: AccessLogEntry }) => h('div', [
      h('div', record.ip),
...
const timePresets = [
  { label: 'Last 15 minutes', value: () => ({ start: dayjs().subtract(15, 'minute'), end: dayjs() }) },
  { label: 'Last 30 minutes', value: () => ({ start: dayjs().subtract(30, 'minute'), end: dayjs() }) },
  { label: 'Last hour', value: () => ({ start: dayjs().subtract(1, 'hour'), end: dayjs() }) },
  { label: 'Last 4 hours', value: () => ({ start: dayjs().subtract(4, 'hour'), end: dayjs() }) },
  { label: 'Last 12 hours', value: () => ({ start: dayjs().subtract(12, 'hour'), end: dayjs() }) },
  { label: 'Last 24 hours', value: () => ({ start: dayjs().subtract(24, 'hour'), end: dayjs() }) },
  { label: 'Last 7 days', value: () => ({ start: dayjs().subtract(7, 'day'), end: dayjs() }) },
  { label: 'Last 30 days', value: () => ({ start: dayjs().subtract(30, 'day'), end: dayjs() }) },
]
    ...
    <div v-else-if="shouldShowContent">
      ...
      <div v-if="shouldShowControls" class="mb-4">
        ...
        <AForm layout="inline" class="mb-4">
          <AFormItem :label="$gettext('Time Range')">
            <ASpace>
      ...
      <div v-else class="text-center" style="padding: 40px;">
        ...
        <div v-if="isReady" class="mt-4">
          ...
          <p v-if="logPath" class="text-gray-400 text-sm mt-1">
            {{ $gettext('Log path') }}: {{ logPath }}
          </p>
