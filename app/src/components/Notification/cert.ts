...
export function syncCertificateError(text: string) {
  ...
  if (data.status_code === 404) {
    return $gettext('Sync Certificate %{cert_name} to %{env_name} failed, please upgrade the remote PrimeWaf to the latest version', { cert_name: data.cert_name, env_name: data.env_name }, true)
  }
