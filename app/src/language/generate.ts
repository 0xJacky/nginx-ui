...
export const msg = [
  ...
  $gettext('Check if Nginx UI can write a temporary file to the configured Nginx directory and the external Nginx container can see it at the same path. Docker socket access and ContainerName only route control commands; configuration and log directories must be mounted into both containers at matching paths.'),
  ...
  $gettext('External Nginx configuration directory is shared'),
