...
import { ENCODED_PATH_PREFIX, encodePathParam, normalizeRequestUrl, PATH_PARAMS_BY_URL } from '@/lib/helper/encodePathParam'
...
/**
 * Base64url-encode any query parameter that carries a filesystem path.
 *
 * Done here rather than at each call site for two reasons: it is one place
 * instead of seven, and two of the affected requests are generated by
 * useCurdApi and have no hand-written call site to patch — the `dir` filter on
 * GET /configs and the `path` search column on GET /nginx_logs.
 *
 * Only GET is transformed. Paths in POST bodies are not in the URL and are left
 * alone.
 */
function encodePathParamsInPlace(config: InternalAxiosRequestConfig) {
  if (config.method?.toLowerCase() !== 'get' || !config.params)
    return

  const params = PATH_PARAMS_BY_URL[normalizeRequestUrl(config.url)]
  if (!params)
    return

  for (const name of params) {
    const value = config.params[name]
    if (typeof value === 'string' && value !== '' && !value.startsWith(ENCODED_PATH_PREFIX))
      config.params[name] = encodePathParam(value)
  }
}
...
export function setupRequestInterceptor() {
  ...
  setRequestInterceptor(
    async config => {
      ...
      encodePathParamsInPlace(config)
