...
var (
	...
	ErrInvalidDDNSIPVersion        = cosy.NewError(40013, "DDNS IP version must be ipv4, ipv6, ipv4_ipv6, ipv6_ipv4, or both_required")
	ErrDDNSIPVersionRecordMismatch = cosy.NewError(40014, "DDNS record type does not match the selected IP version")
	ErrDDNSBothRequiredTarget      = cosy.NewError(40015, "DDNS both_required mode requires at least one A record and one AAAA record")
