...
func (li *LogIndexer) buildSearchQuery(req *QueryRequest) query.Query {
	...
	if !req.StartTime.IsZero() && !req.EndTime.IsZero() {
		...
		if req.EndTime.Sub(req.StartTime) < 400*24*time.Hour { // Less than ~400 days
			logger.Infof("Using time range filter: %s to %s", req.StartTime.Format(time.RFC3339), req.EndTime.Format(time.RFC3339))
			timeQuery := bleve.NewDateRangeQuery(req.StartTime, req.EndTime)
...
func (li *LogIndexer) createCacheKey(req *QueryRequest) string {
	...
	return fmt.Sprintf("search_%s_%s_%s_%s_%s_%s_%s_%s_%s_%s_%s_%s_%d_%d_%s_%s",
