...
func (s *BackgroundLogService) discoverLogFiles(logPath string) {
	...
	if err := s.indexer.DiscoverLogFiles(logDir, baseLogName); err != nil {
		...
		logger.Debugf("Successfully discovered log files for %s", logPath)
		// Send index ready notification
		s.notifyIndexReady(logPath)
	}
}

// notifyIndexReady sends an event notification when index is ready
func (s *BackgroundLogService) notifyIndexReady(logPath string) {
	if s.indexer == nil {
		return
	}

	// Get time range for the indexed log
	start, end := s.indexer.GetTimeRangeForPath(logPath)
	...
	logger.Infof("Sent index ready notification for log path: %s", logPath)
}
