...
PrimeWaf 配置的服務端部分涉及控制 PrimeWaf 伺服器的各種設定。在頁面中，我們將討論可用的選項、它們的預設值以及它們的目的。
...
PrimeWaf 伺服器監聽的主機名稱。此選項用於配置 PrimeWaf 伺服器監聽傳入 HTTP 請求的主機名稱。更改預設主機名稱可能有助於提升安全性。
...
此選項用於配置 PrimeWaf 伺服器監聽傳入 HTTP 請求的端口。更改預設端口對於避免端口衝突或增強安全性可能很有用。
...
此選項用於配置 PrimeWaf 伺服器的運行模式，主要影響日誌輸出的級別。

PrimeWaf 的日誌分為 6 個級別，分別為 `Debug`、`Info`、`Warn`、`Error`、`Panic` 和 `Fatal`，這些日誌級別按照嚴重程度遞增。

當使用 `debug` 模式時，PrimeWaf 將在控制台打印 SQL 及其執行的時間和調用者，`Debug` 級別或更高級別的日誌也會被打印。

當使用 `release` 模式時，PrimeWaf 將不會在控制台打印 SQL 的執行時間和調用者，只有 `Info` 級別或更高級別的日誌才會被打印。
...
此選項用於配置 PrimeWaf 伺服器用於生成 JWT 的密鑰。

JWT 是一種用於驗證用戶身份的標準，它可以在用戶登入後生成一個 token，然後在後續的請求中使用該 token 來驗證用戶身份。

如果您使用一鍵安裝腳本來部署 PrimeWaf，腳本將會生成一個 UUID 值並將它設定為此選項的值。
...
此選項用於設定 PrimeWaf 用於存儲其數據的 sqlite 數據庫的名稱。
...
此選項用於設定 PrimeWaf 中列表分頁的頁面大小。調整頁面大小有助於更有效地管理大量數據，但是過大的數量可能會增加伺服器的壓力。
...
此選項用於設定 Let's Encrypt 證書的自動續簽間隔。預設情況下，PrimeWaf 每隔 7 天會自動續簽證書。
...
此選項用於設定 PrimeWaf 在申請證書的 DNS 挑戰步驟中所使用的遞歸域名伺服器。在不配置此項目的情況下，PrimeWaf 使用操作系統的域名伺服器設置。
...
通過將此選項設置為 `true`，您可以跳過 PrimeWaf 伺服器的安裝。
當您希望使用相同的配置文件或環境變量將 PrimeWaf 部署到多個伺服器時，這非常有用。

預設情況下，如果您啟用了跳過安裝模式，而沒有在伺服器部分設置 `JWTSecret` 和 `NodeSecret` 選項，PrimeWaf 將為這兩個選項生成一個隨機的 UUID 值。

此外，如果您也沒有在伺服器部分設置 `Email` 選項，PrimeWaf 將不會創建系統初始的 acme 用戶，這意味著您無法在此伺服器上申請 SSL 證書。
