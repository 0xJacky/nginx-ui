...
const {{.Name | replaceSpaces}}Config: ExternalNotifyConfig = {
  name: () => $gettext('{{.Title | escapeQuotes}}'),
  config: [
    {{- range .Fields}}
    {
      key: '{{.Key}}',
      label: '{{.Title | escapeQuotes}}',
    },
    {{- end}}
  ],
}
...
// Template for the gettext extraction anchor file.
//
// Field labels are emitted as plain strings in the generated notifier configs
// and translated at render time by ExternalNotifyEditor.vue via
// $gettext(item.label). Because that call sees a variable, `gettext:extract`
// cannot record the msgids from it, so this file restates every label as a
// literal $gettext call purely to feed the extractor. Nothing imports it at
// runtime; the same trick is used by app/src/language/generate.ts.
const tsLabelsTemplate = `// This file is auto-generated by notification generator. DO NOT EDIT.
...
{{- range .}}
  $gettext('{{. | escapeQuotes}}'),
{{- end}}
]
`

// Regular expression to extract @external_notifier annotation.
// The captured value is either a bare identifier or "Identifier|Display Title",
// so a notifier can keep a stable config key while showing a richer label.
var externalNotifierRegex = regexp.MustCompile(`@external_notifier\(([^)\n]+)\)`)

// splitAnnotation separates the notifier identifier from its optional display
// title. The identifier drives the config key, file name and TypeScript symbol,
// so it must stay ASCII; the title is free-form and only reaches $gettext.
func splitAnnotation(annotation string) (name string, title string) {
	name, title, found := strings.Cut(annotation, "|")
...
func GenerateExternalNotifiers(root string) error {
	...
	// Update the gettext extraction anchor for field labels
	if err := generateLabelsFile(notifiers, outputDir); err != nil {
		return fmt.Errorf("error updating labels.ts: %w", err)
	}
		...
		for _, spec := range genDecl.Specs {
			...
			if genDecl.Doc != nil {
				for _, comment := range genDecl.Doc.List {
					...
					if len(matches) > 1 {
						name, title := splitAnnotation(matches[1])
						notifierInfo.Name = name
						notifierInfo.Title = title
						notifierInfo.ConfigKey = strings.ToLower(typeSpec.Name.Name)
						notifierInfo.FileName = strings.ToLower(strings.ReplaceAll(name, " ", "_"))
						found = true
...
// templateFuncs are shared by every generated TypeScript file.
var templateFuncs = template.FuncMap{
	"replaceSpaces": func(s string) string {
		return strings.ReplaceAll(s, " ", "")
	},
	// Display titles and field labels are free-form, so keep them from breaking
	// the single-quoted TypeScript literals they are rendered into.
...
func generateTSConfig(notifier NotifierInfo, outputDir string) error {
	...
	tmpl, err := template.New("tsConfig").Funcs(templateFuncs).Parse(tsConfigTemplate)
	if err != nil {
...
// generateLabelsFile writes a single TypeScript file that lists every notifier
// field label as a $gettext literal. The generated notifier configs store
// labels as plain strings, so without this anchor `gettext:extract` would never
// see the msgids and every label would fall back to English at runtime.
func generateLabelsFile(notifiers []NotifierInfo, outputDir string) error {
	// Deduplicate and sort so the output stays stable across runs.
	seen := make(map[string]bool)
	var labels []string
	for _, notifier := range notifiers {
		for _, field := range notifier.Fields {
			if field.Title == "" || seen[field.Title] {
				continue
			}
			seen[field.Title] = true
			labels = append(labels, field.Title)
		}
	}
	sort.Strings(labels)

	tmpl, err := template.New("tsLabels").Funcs(templateFuncs).Parse(tsLabelsTemplate)
	if err != nil {
		return fmt.Errorf("error creating labels template: %w", err)
	}

	outputFile := filepath.Join(outputDir, "labels.ts")
	file, err := os.Create(outputFile)
	if err != nil {
		return fmt.Errorf("error creating output file %s: %w", outputFile, err)
	}
	...
	if err := tmpl.Execute(file, labels); err != nil {
		return fmt.Errorf("error executing labels template: %w", err)
	}

	logger.Infof("Generated %d notifier field labels at %s\n", len(labels), outputFile)
	return nil
}
